...
namespace Deconf\AIWP\Google\AccessToken;

use DateTime;
use DomainException;
use Exception;
use ExpiredException;
use Deconf\AIWP\Firebase\JWT\ExpiredException as ExpiredExceptionV3;
use Deconf\AIWP\Firebase\JWT\Key;
use Deconf\AIWP\Firebase\JWT\SignatureInvalidException;
use Deconf\AIWP\Google\Auth\Cache\MemoryCacheItemPool;
...
class Verify
{
    ...
    /**
     * @var \Deconf\AIWP\Firebase\JWT\JWT
    */
    public $jwt;
    ...
    {
        $jwtClass = 'JWT';
        if (class_exists('\Deconf\AIWP\Firebase\JWT\JWT')) {
            $jwtClass = 'Deconf\AIWP\Firebase\JWT\JWT';
        }
    ...
    private function getRsaClass()
    {
        ...
        if (class_exists('Deconf\AIWP\phpseclib\Crypt\RSA')) {
    ...
    private function getBigIntClass()
    {
        ...
        if (class_exists('Deconf\AIWP\phpseclib\Math\BigInteger')) {
    ...
    private function getOpenSslConstant()
    {
        ...
        if (class_exists('Deconf\AIWP\phpseclib\Crypt\RSA')) {
            return 'Deconf\AIWP\phpseclib\Crypt\RSA::MODE_OPENSSL';
        }
