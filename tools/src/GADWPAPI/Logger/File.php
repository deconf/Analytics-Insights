 ...
 */
class GADWPAPI_Logger_File extends GADWPAPI_Logger_Abstract
{
  /**
  ...
  public function __construct(GADWPAPI_Client $client)
  {
    ...
    $file = $client->getClassConfig('GADWPAPI_Logger_File', 'file');
    ...
    $mode = $client->getClassConfig('GADWPAPI_Logger_File', 'mode');
    if (!$mode) {
      $this->mode = $mode;
    }

    $this->lock = (bool) $client->getClassConfig('GADWPAPI_Logger_File', 'lock');
    $this->file = $file;
  }
  ...
  protected function write($message)
  {
    if (is_string($this->file)) {
      ...
      throw new GADWPAPI_Logger_Exception('File pointer is no longer available');
    }
